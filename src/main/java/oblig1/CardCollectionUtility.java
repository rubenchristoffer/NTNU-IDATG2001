...
/**
 * Utility class containing useful functions when dealing with collections of
 * cards
 * 
 * @author Ruben
 */
public final class CardCollectionUtility {
	...
	public static void printSpareCards(Collection<Card> cardCollection) {
		cardCollection.stream().filter(card -> card.getSuit() == 'S').forEach(System.out::println); // Card has a
																									// modified toString
																									// mapping to
																									// Card::getDetails,
																									// so this works
	...
	public static List<Card> getHeartCards(Collection<Card> cardCollection) {
		return cardCollection.stream().filter(card -> card.getSuit() == 'H').collect(Collectors.toList());
	}

	/**
	 * Gets a list of all the suits of the cards in correct order.
	 * 
	 * @param cardCollection is the collection of cards
	 * @return list of chars representing the card suits
	 */
	public static List<Character> getCardSuits(Collection<Card> cardCollection) {
		return cardCollection.stream().map(card -> card.getSuit()).collect(Collectors.toList());
	}
	...
	public static int getCardFaceValueSum(Collection<Card> cardCollection) {
		return cardCollection.stream().reduce(0, (sum, card) -> sum + card.getFace(), (int1, int2) -> int1 + int2); // This
																													// works
																													// for
																													// parallel
																													// streams
	...
	public static boolean doesQueenOfSpadesExist(Collection<Card> cardCollection) {
		return cardCollection.stream().anyMatch(card -> card.getFace() == 12 && card.getSuit() == 'S');
	}

	/**
	 * This function checks if the card collection has at least 5 cards of the same
	 * suit
	 * 
	 * @param cardCollection is the collection of cards
	...
	public static boolean hasPokerFlush(Collection<Card> cardCollection) {
		return cardCollection.stream().collect(Collectors.groupingBy(Card::getSuit, Collectors.counting())) // Group
																											// sums of
																											// each suit
																											// together
				.values().stream().anyMatch(count -> count >= 5); // Return if the count of any suit is greater or equal
																	// to 5
	}

	/**
	 * This function checks if the cards collection has a size of at least 5 and
	 * only contains cards of one suit.
	 * 
	 * @param cardCollection is the collection of cards
	 * @return true if there are 5 or more cards in collection and they all have the
	 *         same suit
	 */
	public static boolean isPokerFlush(Collection<Card> cardCollection) {
		return cardCollection.size() >= 5 && // Check if at least 5 cards in collection
				cardCollection.stream().map(card -> card.getSuit()) // Extract suits of cards
