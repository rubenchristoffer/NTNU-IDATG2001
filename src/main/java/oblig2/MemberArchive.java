...
/**
 * MemberArchive is a archive of Bonus Members and contains various methods used
 * to interact with these members.
 * 
...
public class MemberArchive {
	...
	/**
	 * This is the minimum required bonus points required to be upgraded to a silver
	 * member.
	 */
	public static final int SILVER_LIMIT = 25000;

	/**
	 * This is the minimum required bonus points required to be upgraded to a gold
	 * member.
	 */
	...
	/**
	 * Finds the amount of points of a member given membership number and password
	 * in clear-text.
	 * 
	 * @param memberNo is the membership number
	 * @param password is the password in clear-text
	 * @return amount of points this member has or -1 if either member does not
	...
	/**
	 * Registers bonus points for a member based on amount of base points.
	 * 
	 * @param memberNo   is the membership number
	 * @param basePoints is the amount of points that should be awarded before any
	 *                   kind of extra bonuses are accounted for
	 * @return true if registering points were a success, false otherwise (if member
	 ...
	 * @return the randomly generated membership number of the new member
	 */
	...
	/**
	 * Checks if any members qualify for a upgrade and upgrades them accordingly.
	 * 
	...
	private int findAvailableNo() {
		return randomGenerator.ints(0, 1000000).filter(i -> !membersMap.containsKey(i)).findAny().getAsInt();
	}
	...
	private BonusMember tryUpgradingMember(BonusMember member, LocalDate localDate) {
		...
		if (points >= GOLD_LIMIT && !(member instanceof GoldMember))
			return new GoldMember(member.getMemberNo(), member.getPersonals(), member.getEnrolledDate(),
					member.getBonuspoints());
		if (points >= SILVER_LIMIT && member instanceof BasicMember)
			return new SilverMember(member.getMemberNo(), member.getPersonals(), member.getEnrolledDate(),
					member.getBonuspoints());
